...
python precompute_semantic_ids_offset.py --features 81 82 --data_type both --use_full_coverage
"""
...
def comprehensive_semantic_id_check(semantic_id_dict, feature_id):
    ...
    sample_ids = sorted(list(semantic_id_dict.keys()), key=str)[:10]
...
def collect_creative_ids_from_data(data_paths, data_type="train", include_full_indexer=True, include_candidate_set=True, include_item_pool=True):
    """
    🔧 修复版：全面收集creative_id，确保覆盖训练、推理、候选集、全量物品池
    
    Args:
        data_paths: 数据文件路径列表
        data_type: 数据类型 ("train" 或 "eval")
        include_full_indexer: 是否包含indexer中的全量物品ID（推荐True）
        include_candidate_set: 是否包含候选集文件（eval时推荐True）
        include_item_pool: 是否包含item_feat_dict.json中的全部物品（推荐True）
    
    Returns:
        tuple: (original_creative_ids_set, indexer_dict, coverage_stats)
            - original_creative_ids_set: 原始creative_id的集合（字符串格式）
            - indexer_dict: indexer字典，用于后续映射
            - coverage_stats: 覆盖统计信息字典
    """
    # 🔧 加载indexer进行ID映射
    if data_type == "train":
    ...
    # 🔧 新增：收集不同来源的creative_id集合
    coverage_stats = {
        'seq_creative_ids': set(),      # 来自交互序列
        'candidate_creative_ids': set(), # 来自候选集
        'item_pool_creative_ids': set(), # 来自item特征池
        'indexer_creative_ids': set(),   # 来自indexer全量
        'final_union_creative_ids': set() # 最终并集
    }
    
    print(f"\n🎯 开始全面收集creative_id ({data_type})...")
    
    # 第一部分：从交互序列数据收集
    print(f"📂 第一步：从交互序列收集creative_id...")
    ...
    # 转换为原始creative_id（字符串）
    for item_reid in creative_ids_reid:
        if item_reid in indexer_i_rev:
            original_creative_id = indexer_i_rev[item_reid]
            coverage_stats['seq_creative_ids'].add(original_creative_id)
    
    print(f"📊 交互序列收集到: {len(coverage_stats['seq_creative_ids'])} 个creative_id")
    
    # 第二部分：收集候选集数据（仅在eval模式且启用时）
    if include_candidate_set and data_type == "eval":
        print(f"📂 第二步：从候选集收集creative_id...")
        candidate_file = Path(data_dir) / 'predict_set.jsonl'
        if candidate_file.exists():
            print(f"📂 处理候选集文件: {candidate_file}")
            line_count = 0
            with open(candidate_file, 'r') as f:
                for line in tqdm(f, desc="扫描候选集"):
                    line_count += 1
                    try:
                        data = json.loads(line)
                        ...
                        # 🔧 调试：显示前几行的数据格式
                        if line_count <= 3:
                            print(f"   候选集样本 {line_count}: keys={list(data.keys())}")
                            if 'creative_id' in data:
                                print(f"      creative_id={data['creative_id']} (type: {type(data['creative_id'])})")
                        
                        # 根据infer.py的格式，候选集数据应该有creative_id字段
                        if 'creative_id' in data:
                            candidate_id = data['creative_id']
                            # 🔧 修复：统一使用整数类型，与indexer保持一致
                            # 在infer.py中：item_id = indexer[creative_id]，这意味着creative_id是原始ID
                            if isinstance(candidate_id, str):
                                try:
                                    candidate_id_int = int(candidate_id)
                                    coverage_stats['candidate_creative_ids'].add(candidate_id_int)
                                    if line_count <= 3:
                                        print(f"      ✅ 添加字符串原始ID: {candidate_id} -> {candidate_id_int}")
                                except ValueError:
                                    # 字符串无法转换为整数，跳过
                                    if line_count <= 3:
                                        print(f"      ❌ 字符串ID无法转换为整数: {candidate_id}")
                                    continue
                            elif isinstance(candidate_id, int):
                                # 整数形式的原始creative_id，直接使用
                                coverage_stats['candidate_creative_ids'].add(candidate_id)
                                if line_count <= 3:
                                    print(f"      ✅ 添加整数原始ID: {candidate_id}")
                        else:
                            # 备用：检查其他可能的字段名
                            found = False
                            for key in ['item_id', 'id', 'itemId']:
                                if key in data:
                                    candidate_id = data[key]
                                    # 🔧 修复：统一使用整数类型，与indexer保持一致
                                    if isinstance(candidate_id, str):
                                        try:
                                            candidate_id_int = int(candidate_id)
                            ...
                            if not found and line_count <= 5:
                                print(f"   ⚠️ 候选集第{line_count}行未找到creative_id字段: {list(data.keys())}")
                    
                    except json.JSONDecodeError as e:
                        print(f"   ❌ 候选集第{line_count}行JSON解析失败: {e}")
                        continue
            
            print(f"📊 候选集总行数: {line_count}")
            print(f"📊 候选集收集到: {len(coverage_stats['candidate_creative_ids'])} 个creative_id")
            
            # 显示收集到的creative_id示例
            if len(coverage_stats['candidate_creative_ids']) > 0:
                sample_ids = sorted(list(coverage_stats['candidate_creative_ids']), key=str)[:10]
                print(f"📋 候选集creative_id示例: {sample_ids}")
            
        else:
            print(f"⚠️  候选集文件不存在: {candidate_file}")
    elif include_candidate_set and data_type == "train":
        print(f"📋 训练模式跳过候选集收集")
    else:
        print(f"📋 候选集收集已禁用")
    ...
    if include_item_pool:
        print(f"📂 第三步：从item特征池收集creative_id...")
        item_feat_file = Path(data_dir) / 'item_feat_dict.json'
        if item_feat_file.exists():
            print(f"📂 处理item特征池: {item_feat_file}")
            with open(item_feat_file, 'r') as f:
                item_feat_dict = json.load(f)
                # 🔧 修复：item_feat_dict的key是字符串化的整型ID（reid），需要转换为原始creative_id
                for reid_str in tqdm(item_feat_dict.keys(), desc="扫描特征池"):
                    try:
                        # 将字符串ID转换为整型reid
                        reid = int(reid_str)
                        # 通过反向映射获取原始creative_id
                        if reid in indexer_i_rev:
                            original_creative_id = indexer_i_rev[reid]
                            coverage_stats['item_pool_creative_ids'].add(original_creative_id)
                    except (ValueError, KeyError):
                        # 如果转换失败或映射不存在，跳过
                        continue
            print(f"📊 特征池收集到: {len(coverage_stats['item_pool_creative_ids'])} 个creative_id")
        else:
            print(f"⚠️  item特征池文件不存在: {item_feat_file}")
    else:
        print(f"📋 item特征池收集已禁用")
    
    # 第四部分：收集indexer全量数据
    if include_full_indexer:
        print(f"📂 第四步：从indexer收集全量creative_id...")
        for creative_id in tqdm(indexer['i'].keys(), desc="扫描indexer"):
            coverage_stats['indexer_creative_ids'].add(creative_id)
        print(f"📊 indexer全量收集到: {len(coverage_stats['indexer_creative_ids'])} 个creative_id")
    else:
        print(f"📋 indexer全量收集已禁用")
    
    # 第五部分：计算最终并集
    print(f"📊 第五步：计算最终并集...")
    final_creative_ids = set()
    
    # 合并所有来源
    final_creative_ids.update(coverage_stats['seq_creative_ids'])
    final_creative_ids.update(coverage_stats['candidate_creative_ids'])
    final_creative_ids.update(coverage_stats['item_pool_creative_ids'])
    final_creative_ids.update(coverage_stats['indexer_creative_ids'])
    
    coverage_stats['final_union_creative_ids'] = final_creative_ids
    
    # 🔧 详细的覆盖统计报告
    print(f"\n📊 ===== 覆盖统计报告 ({data_type}) =====")
    print(f"   交互序列: {len(coverage_stats['seq_creative_ids'])} 个")
    print(f"   候选集: {len(coverage_stats['candidate_creative_ids'])} 个")
    print(f"   特征池: {len(coverage_stats['item_pool_creative_ids'])} 个")
    print(f"   indexer全量: {len(coverage_stats['indexer_creative_ids'])} 个")
    print(f"   最终并集: {len(coverage_stats['final_union_creative_ids'])} 个")
    
    # 分析重叠情况
    seq_only = coverage_stats['seq_creative_ids'] - coverage_stats['indexer_creative_ids']
    candidate_only = coverage_stats['candidate_creative_ids'] - coverage_stats['seq_creative_ids']
    pool_only = coverage_stats['item_pool_creative_ids'] - coverage_stats['seq_creative_ids']
    
    print(f"\n📊 ===== 重叠分析 =====")
    print(f"   仅在交互序列: {len(seq_only)} 个")
    print(f"   仅在候选集: {len(candidate_only)} 个")
    print(f"   仅在特征池: {len(pool_only)} 个")
    
    if len(candidate_only) > 0:
        print(f"   🎯 候选集独有示例: {sorted(list(candidate_only), key=str)[:5]}")
    if len(pool_only) > 0:
        print(f"   🎯 特征池独有示例: {sorted(list(pool_only), key=str)[:5]}")
    
    # 覆盖率分析
    if len(coverage_stats['indexer_creative_ids']) > 0:
        seq_coverage = len(coverage_stats['seq_creative_ids']) / len(coverage_stats['indexer_creative_ids'])
        print(f"   交互序列覆盖率: {seq_coverage:.1%}")
    
    print(f"📋 最终creative_id示例: {sorted(list(final_creative_ids), key=str)[:10]}")
    
    return final_creative_ids, indexer, coverage_stats
...
def precompute_semantic_ids_for_feature(feature_id, creative_ids, data_type="train", mm_emb_path=None, device='cuda', batch_size=1024, use_training_strategy=True):
    ...
    if valid_creative_ids:
        sample_creative_ids = sorted(list(valid_creative_ids), key=str)
    ...
    if valid_creative_ids:
        valid_examples = sorted(list(valid_creative_ids), key=str)[:20]  # 取前20个
    ...
    if missing_creative_ids:
        missing_examples = sorted(list(missing_creative_ids), key=str)[:10]  # 取前10个
    ...
    if use_training_strategy:
        ...
        if len(valid_creative_ids_list) > 0:
            print(f"📋 有效creative_id范围: [{min(valid_creative_ids_list, key=str)}, {max(valid_creative_ids_list, key=str)}]")
        ...
        if missing_creative_ids:
            ...
            if len(missing_creative_ids_list) > 0:
                print(f"📋 缺失creative_id范围: [{min(missing_creative_ids_list, key=str)}, {max(missing_creative_ids_list, key=str)}]")
                print(f"📋 缺失creative_id示例: {sorted(missing_creative_ids_list, key=str)[:10]}")
...
def save_semantic_id_dict(semantic_id_dict, feature_id, data_type="train", use_full_coverage=True):
    """
    ...
    Args:
        ...
        feature_id: 特征ID
        data_type: 数据类型 ("train" 或 "eval")
        use_full_coverage: 是否使用全覆盖版本命名（_full后缀）
    """
    cache_dir = os.environ.get('USER_CACHE_PATH')
    if not cache_dir:
    ...
    # 分离训练和推理的缓存文件
    if data_type == "train":
        semantic_file = cache_path / f"semantic_id_{feature_id}_train{suffix}.pkl"
        json_file = cache_path / f"semantic_id_{feature_id}_train{suffix}.json"
    else:  # eval
        semantic_file = cache_path / f"semantic_id_{feature_id}_eval{suffix}.pkl"
        json_file = cache_path / f"semantic_id_{feature_id}_eval{suffix}.json"
    ...
    print(f"💾 semantic_id字典已保存 ({data_type}, {version_desc}):")
...
def infer():
    ...
    parser.add_argument('--features', nargs='+', default=['84'], 
                       choices=['81', '82', '83', '84', '85', '86'], 
                       help='要预计算的特征ID')
    parser.add_argument('--device', default='cuda', help='计算设备')
    parser.add_argument('--batch_size', type=int, default=8292, help='批次大小')
    ...
    # 🎯 新增：覆盖策略控制参数
    parser.add_argument('--include_full_indexer', action='store_true', default=True,
                       help='包含indexer中的全量物品ID（推荐启用，确保完整覆盖）')
    parser.add_argument('--include_candidate_set', action='store_true', default=True,
                       help='包含候选集文件predict_set.jsonl（eval时推荐启用）')
    parser.add_argument('--include_item_pool', action='store_true', default=True,
                       help='包含item_feat_dict.json中的全部物品（推荐启用）')
    parser.add_argument('--use_full_coverage', action='store_true', default=True,
                       help='启用全覆盖模式，生成_full版本的缓存文件（推荐启用）')
    
    # 兼容性参数
    parser.add_argument('--legacy_mode', action='store_true', default=False,
                       help='使用旧版覆盖策略（仅从交互序列收集，不推荐）')
    ...
    print(f"🎯 覆盖策略配置:")
    print(f"   全量indexer: {args.include_full_indexer}")
    print(f"   候选集: {args.include_candidate_set}")
    print(f"   特征池: {args.include_item_pool}")
    print(f"   全覆盖模式: {args.use_full_coverage}")
    print(f"   兼容模式: {args.legacy_mode}")
    
    if args.legacy_mode:
        print(f"⚠️  警告: 使用旧版覆盖策略，可能导致冷启动物品语义ID缺失")
        args.include_full_indexer = False
        args.include_candidate_set = False
        args.include_item_pool = False
        args.use_full_coverage = False
    ...
    for data_type in data_types_to_process:
        ...
        creative_ids, indexer, coverage_stats = collect_creative_ids_from_data(
            data_files, 
            data_type,
            include_full_indexer=args.include_full_indexer,
            include_candidate_set=args.include_candidate_set,
            include_item_pool=args.include_item_pool
        )
        ...
        for feature_id in args.features:
            ...
            # 🔧 修改缓存文件名检查逻辑
            if args.use_full_coverage:
                cache_file = Path(cache_dir) / f"semantic_id_{feature_id}_{data_type}_full.pkl"
                version_desc = "全覆盖版本"
            else:
            ...
            if cache_file.exists() and not args.force_recompute:
                print(f"📋 特征 {feature_id} ({data_type}, {version_desc}) 的semantic_id缓存已存在: {cache_file}")
            ...
            print(f"\n🎯 开始处理特征 {feature_id} ({data_type}, {version_desc})...")
            ...
            try:
                ...
                save_semantic_id_dict(semantic_id_dict, feature_id, data_type, use_full_coverage=args.use_full_coverage)
                
                print(f"✅ 特征 {feature_id} ({data_type}, {version_desc}) 预计算完成")
                ...
                # 🎯 输出覆盖改进统计
                if 'coverage_stats' in locals():
                    print(f"📊 覆盖改进效果:")
                    seq_count = len(coverage_stats['seq_creative_ids'])
                    total_count = len(coverage_stats['final_union_creative_ids'])
                    improvement = ((total_count - seq_count) / max(seq_count, 1)) * 100
                    print(f"   交互序列覆盖: {seq_count} -> 全覆盖: {total_count}")
                    print(f"   覆盖改进: +{total_count - seq_count} 个ID (+{improvement:.1f}%)")
                    print(f"   冷启动/候选集覆盖得到显著改善！")
    ...
    print("="*60)
    print("💡 使用方法:")
    print("   训练时使用: --use_precomputed_semantic_ids (会自动优先加载_full版本)")
    print("   推理时使用: --use_precomputed_semantic_ids (会自动优先加载_full版本)")
    print("🚀 预期效果:")
    print("   - 训练和推理速度将大幅提升")
    print("   - 显存占用也会减少")
    print("   - 避免重复的RQ-VAE编码计算")
    print("   - 🎯 冷启动和候选集物品现在都有真实的语义ID！")
    ...
    if cache_dir:
        cache_path = Path(cache_dir)
        # 优先显示_full版本文件
        semantic_files_full = list(cache_path.glob("semantic_id_*_full.pkl"))
        semantic_files_regular = list(cache_path.glob("semantic_id_*.pkl"))
        semantic_files_regular = [f for f in semantic_files_regular if '_full.pkl' not in f.name]
        
        if semantic_files_full or semantic_files_regular:
            print(f"\n📊 生成的缓存文件统计:")
            total_size = 0
            
            if semantic_files_full:
                print(f"   🎯 全覆盖版本文件:")
                for file in semantic_files_full:
                    file_size = file.stat().st_size / (1024 * 1024)  # MB
                    total_size += file_size
                    print(f"      {file.name}: {file_size:.2f} MB")
            
            if semantic_files_regular:
                print(f"   📋 常规版本文件:")
                for file in semantic_files_regular:
                    file_size = file.stat().st_size / (1024 * 1024)  # MB
                    total_size += file_size
                    print(f"      {file.name}: {file_size:.2f} MB")
            ...
            if semantic_files_full:
                print(f"\n✅ 建议使用全覆盖版本以获得最佳的冷启动和候选集覆盖！")
            elif semantic_files_regular:
                print(f"\n⚠️  当前仅有常规版本，建议重新运行生成全覆盖版本：")
                print(f"   python precompute_semantic_ids.py --features {' '.join(args.features)} --data_type {args.data_type} --use_full_coverage")
